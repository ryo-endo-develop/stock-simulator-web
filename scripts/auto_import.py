...
class AIResponseParser:
    ...
    def extract_toyota_prediction(self, content: str) -> dict:
        ...
        try:
            ...
            return {"predicted_price": predicted_price, "notes": "AI自動解析による予測"}

        except Exception:
            return {"predicted_price": 3000.0, "notes": "予測抽出エラー"}
...
class AutoImporter:
        ...
        for model_id, prediction in self.parser.predictions.items():
            ...
            if prediction["toyota"]:
                ...
                if success:
                    total_imported += 1
                    print(
                        f"  ✅ トヨタ予測: ¥{prediction['toyota']['predicted_price']:,.0f}"
                    )
                else:
                    print("  ❌ トヨタ予測エラー")
    ...
    def import_toyota_prediction(self, execution_date, model_id, toyota_data, buy_date):
        ...
        try:
            ...
            sell_date = (buy_date_obj + timedelta(days=4)).strftime(
                "%Y-%m-%d"
            )  # 金曜日
