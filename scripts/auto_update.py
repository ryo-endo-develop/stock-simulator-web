...
class ResultUpdater:
    ...
    def update_fixed_stock_results(self):
        ...
        pending_data = fixed_df[
            ...
            & (
                fixed_df["execution_date"].dt.date
                >= self.target_date.date() - timedelta(days=7)
            )
        ]
        ...
        for _, row in pending_data.iterrows():
            try:
                # 実際の株価を取得
                buy_price, actual_buy_date = (
                    StockAnalyzer.get_closest_business_day_price(
                        row["stock_code"], row["buy_date"]
                    )
                )
                sell_price, actual_sell_date = (
                    StockAnalyzer.get_closest_business_day_price(
                ...
                if buy_price and sell_price:
                    ...
                    return_rate = StockAnalyzer.calculate_return_rate(
                    ...
                    if success:
                        updated_count += 1
                        print(
                            f"  ✅ {row['stock_code']}: ¥{buy_price:,.0f} → ¥{sell_price:,.0f} ({return_rate:+.1f}%)"
                        )
                    else:
    ...
    def update_selection_results(self):
        ...
        pending_data = selection_df[
            ...
            & (
                selection_df["execution_date"].dt.date
                >= self.target_date.date() - timedelta(days=7)
            )
        ]
